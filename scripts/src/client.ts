 ...
 */
const PROGRAM_SO_PATH = path.join(SOLANA_PROGRAM_PATH, 'solana_bank_example.so');

/**
 * Path to the keypair of the deployed program.
 * This file is created when running `solana program deploy dist/program/solana_bank_example.so`
 */
const PROGRAM_KEYPAIR_PATH = path.join(SOLANA_PROGRAM_PATH, 'solana_bank_example-keypair.json');
...
export async function establish_payer(): Promise<void> {
  ...
  if (!payer) {
    ...
    fees += await connection.getMinimumBalanceForRentExemption(MONEY_RECEIVER_ACCOUNT_SIZE);
...
export async function check_program(): Promise<void> {
  ...
  try {
    const program_keypair = await create_keypair_from_file(PROGRAM_KEYPAIR_PATH);
  ...
  const money_receiver_account = await connection.getAccountInfo(money_received_pubkey);
...
export async function deposit_token(): Promise<void> {
  ...
  const instruction = new TransactionInstruction({
    keys: [{pubkey: money_received_pubkey, isSigner: false, isWritable: true}],
    programId: program_id,
    data: Buffer.from(Uint8Array.of(0, ...deposit_amount_big_number, ...deposit_note)),
  });
